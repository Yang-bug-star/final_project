...
class RNN_cell(nn.Module):
    def __init__(self, layers, hidden_unit,num_features, num_classes, layer_limit,actions):
        #hidden_layers: activations for a list，length of list = number of nodes + 1
        #DAGS: a list to store a directed acyclic graph with numbers indicating father nodes, length of it = number of nodes
        super(RNN_cell, self).__init__()
        self.hidd_unit = hidden_unit
        self.linear_layers = []# store linear layer
        self.hidden_layers = []# store activations
        self.DAG = []
        self.actions = actions #controller's actions
        max_layers = 12
        ...
        self.linear_layers.append(
            nn.Linear(in_features=num_features, out_features=self.hidd_unit))#add W(X) first
        for i, layer in enumerate(layers):
        # construct network given the action
            if layer == 'EOS':
                break
    ...
    def compute_val(self, x, h = None):
        ...
        out_layers = {}
        # flag indicating whether output is used
        output_used = [False
                       for u in range(len(self.linear_layers))]
    ...
    def forward(self,word2vec,h,batch_input):
        for l in range(1,int(batch_input.shape[1])):# interation over RNN_cell
...
#         X_tr, y_tr, X_val, y_val, X_te, y_te, word2vec = create_dataset()
    ...
    #save the model in order to realize parameters sharing
    ...
    #compute reward and return it to the controller
    def compute_reward(self, layers, num_epochs,actions,train_loader,validation_loader):
        train_losses = []
        val_accuracies = []
        patience = 10
        hidden_unit = 8
        ...
        for e in range(num_epochs):
            ...
            for batch_idx, (x, y) in enumerate(train_loader):#train child network
        # predict by running forward pass
            ...
            acc = 0
            for batch_idx, (x, y) in enumerate(validation_loader):#evaluate childnetwork
                #AFTER TRAINING
                h = None
                ...
                acc += val_output.eq(batch_targets.view_as(val_output)).sum().item() 
    ...
    def test_accurancy(self, layers, actions,test_loader):#testing module, test final accuracy
        ...
        for batch_idx, (x, y) in enumerate(test_loader):
            ...
            acc += test_output.eq(batch_targets.view_as(test_output)).sum().item()  
