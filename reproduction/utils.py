...
def indexes_to_actions(actions, batch_size, total_actions):
    ...
    batch_hid_units = [] # hidden units in one batch
    batch_act_functions = [] # activation functions avalible in one batch
    batch_index_eos = []
#     print(actions)
#     print(total_actions)
    for b in range(batch_size):
        ...
        try:# EOS reached, cut here
            index_eos = hid_units.index('EOS')
            hid_units = hid_units[:index_eos + 1]
        # if EOS isn't produced, then add EOS by ourselves
        except ValueError:
